...
export class NotificationPublisherService implements OnModuleDestroy {
  ...
  private readonly topic =
    process.env.NOTIFICATIONS_EMAIL_TOPIC || 'notification.email.requested.v1';
  private readonly brokers = (process.env.NOTIFICATIONS_KAFKA_BROKERS || '')
    .split(',')
  ...
  private async getProducer(): Promise<Producer> {
    ...
    const kafka = new Kafka({
      clientId: process.env.OTEL_SERVICE_NAME || 'cv-api',
