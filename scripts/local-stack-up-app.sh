...
if [ $i -gt 60 ]; then
  echo "OpenBao did not become ready in time. Start ops first with npm run local:up:ops." >&2
  exit 1
fi
...
' <<<"$mounts_json"; then
  echo "OpenBao mount '$mount_path' does not exist. Run npm run local:up:ops first." >&2
  exit 1
fi
