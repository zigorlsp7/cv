...
STACK_SERVICES=(postgres_test api_test web)
STARTED_BY_HOOK=0
...
i=1
while [ $i -le 60 ]; do
  if curl -fsS http://localhost:3003/v1/health/ready > /dev/null; then
    echo "API test service is healthy"
    break
  fi
  sleep 2
  i=$((i + 1))
...
if [ $i -gt 60 ]; then
  echo "API test service did not become healthy in time" >&2
  docker compose -f "$COMPOSE_FILE" logs --no-color api_test
  exit 1
fi
...
i=1
while [ $i -le 60 ]; do
  if curl -fsS http://localhost:3101 > /dev/null; then
